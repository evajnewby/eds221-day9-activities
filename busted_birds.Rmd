...
unique(birds_1$species_id) #pipe not useful here, use $ instead
...
bird_totals_by_park <- birds_df %>% 
  left_join(sites_1) %>% 
  group_by(park_name) %>% # changed groupby to group_by
...
ggplot(data = bird_totals_by_park, aes(x = total_count, y = park_name)) +
  ...
  labs(x = "Total bird count",
       y = "Park name") + # remove the pipe and add the plus 
  theme_minimal()
```
...
common_birds <- birds_df %>% 
  full_join(sites_1) %>% #use full join
...
#Don't see any palamino in here, correct spelling is "palomino"
```
